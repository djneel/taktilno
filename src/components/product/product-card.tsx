...
export function ProductCard({
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl bg-card ring-1 ring-line/60 transition-all duration-500 md:hover:-translate-y-1 md:hover:ring-line">
      <Link href={`/product/${product.slug}`} className="relative block aspect-square overflow-hidden bg-bg2">
        {img ? (
          <Image
            src={img.url}
        ...
        <div className="absolute left-3 top-3 flex gap-1.5">
          {product.isNew && (
            <span className="rounded-full bg-pink/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-bg">
              Новинка
            </span>
          )}
          {soldOut && (
            <span className="rounded-full bg-bg/80 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-muted backdrop-blur">
              Нет в наличии
            </span>
          )}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4 sm:p-5">
        ...
        <Link href={`/product/${product.slug}`} className="text-base font-bold leading-tight sm:text-lg">
