...
export function ImageManager({ productId, images: initial }: { productId: number; images: AdminImage[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {images.length === 0 ? <p className="text-sm text-muted">Фотографий пока нет.</p> : (
          ...
          {images.map((img, i) => (
            <li key={img.id} className="overflow-hidden rounded-2xl bg-bg2 ring-1 ring-line/60">
              <div className="relative aspect-square">
                ...
                {img.kind === "main" && <span className="absolute left-2 top-2 rounded-full bg-green px-2 py-0.5 text-[10px] font-bold uppercase text-bg">Главное</span>}
