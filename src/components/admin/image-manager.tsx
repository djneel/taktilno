    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-line p-4 sm:flex-row sm:items-center">
        <div className="flex-1">
          ...
          <div className="text-xs text-muted">JPG, PNG, WebP до 10 МБ. Можно выбрать несколько. Первое фото товара автоматически становится главным.</div>
        </div>
        <select
          value={kind}
        ...
        <Button type="button" variant="green" disabled={uploading} onClick={() => fileRef.current?.click()}>
      ...
      {images.length === 0 ? (
        ...
        <ul className={cn("grid gap-3 sm:grid-cols-2 xl:grid-cols-3", pending && "opacity-70")}>
          {images.map((img, i) => (
            <li key={img.id} className="overflow-hidden rounded-2xl bg-bg2 ring-1 ring-line/60">
              <div className="relative aspect-square">
                <Image src={img.url} alt={img.alt} fill sizes="300px" className="object-cover" />
                {img.kind === "main" && (
                  <span className="absolute left-2 top-2 rounded-full bg-green px-2 py-0.5 text-[10px] font-bold uppercase text-bg">
                    Главное
                  </span>
                )}
                <span className="absolute right-2 top-2 rounded-full bg-bg/70 px-2 py-0.5 text-[10px] font-bold text-fg">
                  {i + 1}
                </span>
              </div>
              <div className="space-y-2 p-3">
                <select
                  ...
                  className="h-10 w-full rounded-lg bg-card px-2 text-sm ring-1 ring-line/60 outline-none"
                  aria-label="Тип фото"
                >
                  {IMAGE_KINDS.map((k) => (
                    <option key={k} value={k}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {img.kind !== "main" && (
                    <button type="button" onClick={() => setMain(img.id)} className="h-9 rounded-full bg-card px-3 text-xs font-bold ring-1 ring-line/60 hover:text-green">
                      Сделать главным
                    </button>
                  )}
                  <button type="button" onClick={() => move(i, -1)} disabled={i === 0} className="h-9 w-9 rounded-full bg-card text-sm ring-1 ring-line/60 disabled:opacity-30" aria-label="Выше">
                    ←
                  </button>
                  <button type="button" onClick={() => move(i, 1)} disabled={i === images.length - 1} className="h-9 w-9 rounded-full bg-card text-sm ring-1 ring-line/60 disabled:opacity-30" aria-label="Ниже">
                    →
                  </button>
                  <button type="button" onClick={() => remove(img.id)} className="ml-auto h-9 rounded-full bg-pink/10 px-3 text-xs font-bold text-pink">
                    Удалить
                  </button>
                </div>
              </div>
