...
export async function updateImageAction(input: { id: number; kind?: ImageKind; alt?: string; colorVariant?: string | null }) {
  ...
  if (input.colorVariant === null || typeof input.colorVariant === "string") patch.colorVariant = input.colorVariant;
...
export async function setMainImageAction(input: { productId: number; imageId: number }) {
  await requireAdmin();
  await db.update(productImages).set({ kind: "front" }).where(and(eq(productImages.productId, input.productId), eq(productImages.kind, "main")));
...
export async function reorderImagesAction(input: { productId: number; ids: number[] }) {
  ...
  for (let i = 0; i < input.ids.length; i++) {
    await db.update(productImages).set({ sortOrder: i }).where(and(eq(productImages.id, input.ids[i]), eq(productImages.productId, input.productId)));
  }
  revalidateShop();
}
...
export async function deleteImageAction(input: { id: number }) {
  ...
  if (img?.mediaId) await db.delete(media).where(eq(media.id, img.mediaId));
  if (img) await normalizeOrder(img.productId);
  revalidateShop();
}

async function normalizeOrder(productId: number) {
  const imgs = await db.query.productImages.findMany({ where: eq(productImages.productId, productId), orderBy: (t, { asc }) => [asc(t.sortOrder), asc(t.id)] });
  const main = imgs.find((i) => i.kind === "main");
  const ordered = main ? [main, ...imgs.filter((i) => i.id !== main.id)] : imgs;
  for (let i = 0; i < ordered.length; i++) await db.update(productImages).set({ sortOrder: i }).where(eq(productImages.id, ordered[i].id));
}
...
export async function saveCategoryAction(formData: FormData) {
  ...
  const name = str(formData, "name");
  if (!name) return;
  const data = { name, slug: slugify(str(formData, "slug") || name) || `cat-${Date.now()}`, description: str(formData, "description"), imageUrl: str(formData, "imageUrl") || null, sortOrder: Math.round(num(formData, "sortOrder")), isActive: bool(formData, "isActive"), showOnHome: bool(formData, "showOnHome") };
  if (id) await db.update(categories).set(data).where(eq(categories.id, id)); else await db.insert(categories).values(data);
  revalidateShop(); revalidatePath("/admin/categories");
}

export async function deleteCategoryAction(formData: FormData) {
  await requireAdmin(); const id = num(formData, "id"); if (!id) return;
  await db.delete(categories).where(eq(categories.id, id)); revalidateShop(); revalidatePath("/admin/categories");
}

export async function moveCategoryAction(formData: FormData) {
  await requireAdmin(); const id = num(formData, "id"); const dir = str(formData, "dir") === "up" ? -1 : 1;
  const all = await db.query.categories.findMany({ orderBy: (t, { asc }) => [asc(t.sortOrder), asc(t.id)] });
  const idx = all.findIndex((c) => c.id === id); const swap = idx + dir;
  if (idx < 0 || swap < 0 || swap >= all.length) return;
  [all[idx], all[swap]] = [all[swap], all[idx]];
  for (let i = 0; i < all.length; i++) await db.update(categories).set({ sortOrder: i + 1 }).where(eq(categories.id, all[i].id));
  revalidateShop(); revalidatePath("/admin/categories");
}

export async function saveReviewAction(formData: FormData) {
  await requireAdmin(); const id = num(formData, "id", 0); const text = str(formData, "text"); const authorName = str(formData, "authorName");
  if (!text || !authorName) return;
  const productIdRaw = num(formData, "productId", 0);
  const data = { authorName, text, rating: Math.min(5, Math.max(1, Math.round(num(formData, "rating", 5)))), productId: productIdRaw > 0 ? productIdRaw : null, photoUrl: str(formData, "photoUrl") || null, isVisible: bool(formData, "isVisible"), sortOrder: Math.round(num(formData, "sortOrder")) };
  if (id) await db.update(reviews).set(data).where(eq(reviews.id, id)); else await db.insert(reviews).values(data);
  revalidateShop(); revalidatePath("/admin/reviews"); redirect("/admin/reviews");
}

export async function toggleReviewAction(formData: FormData) {
  await requireAdmin(); const id = num(formData, "id");
  await db.update(reviews).set({ isVisible: sql`NOT ${reviews.isVisible}` }).where(eq(reviews.id, id)); revalidateShop(); revalidatePath("/admin/reviews");
}

export async function deleteReviewAction(formData: FormData) {
  await requireAdmin(); const id = num(formData, "id"); await db.delete(reviews).where(eq(reviews.id, id)); revalidateShop(); revalidatePath("/admin/reviews");
}
...
export async function saveSettingsAction(formData: FormData) {
  await requireAdmin();
  const keys = ["hero_product_slug", "hero_image_url", "tactile_image_url", "viewer_product_slug", "contact_telegram", "contact_vk", "contact_email", "contact_phone"];
